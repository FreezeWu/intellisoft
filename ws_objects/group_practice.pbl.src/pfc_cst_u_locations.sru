...
type cb_search_old from commandbutton within tabpage_browse
end type
...
this.cb_search_old=create cb_search_old
...
//	$<modification> Add Appeon Queue labels to reduce client-server interactions to improve runtime performance.
	...
	*/
	...
	If ib_tabpage_ass_prac_created Then tab_2.tabpage_ass_prac.dw_prac.retrieve(ll_gp_id)
	If ib_tabpage_survey_created Then tab_2.tabpage_survey.uo_survey.of_reset()//maha 061603
	if tab_2.tabpage_docs.visible = true then tab_2.tabpage_docs.dw_docs.retrieve(ll_gp_id)  //Start Code Change ----10.05.2016 #V153 maha -for documents
...
/*if rb_l_p.checked = true then //practitoners and locations
	...
	//also modifies some of the field names in d_group_prac_search_all to work correctly with the filters
		...
		ls_criteria = dw_search.GetItemString( i, "criteria" )
		ls_criteria = inv_string.of_globalreplace(ls_criteria, "'", "~~'")		
		ls_where = ls_where + "(Upper(" + stype + ") like '" + Upper(ls_criteria) +"%') AND "				
	...
	FOR i = 1 TO li_rc
		...
		if stype = "Specialty" or stype = "Language" or stype = "Provider_type" then
			...
			*/
			IF gs_dbtype = "SQL" THEN
				...
				IF Lower(stype) = "country" THEN stype = "code_lookup_c.description"
			END IF
			//-------Begin Modified by Alfee 01.28.2010 ---------------------
			ls_criteria = dw_search.GetItemString( i, "criteria" )
			ls_criteria = of_reviseexpvalue(ls_criteria, "'")		
			ls_where = ls_where + stype + " like '" + ls_criteria +"%' AND "
			//ls_where = ls_where + stype + " like '" + dw_search.GetItemString( i, "criteria" ) +"%' AND "
			//-------End Modified --------------------------------------------						//---------------------------- APPEON END ----------------------------
		END FOR
	   ...
	   // mskinner 09 Jan 2005 -- begin
		...
		if len(trim(ls_where)) > 0 then 
			messagebox("sql",is_sql + '' + ls_where)
