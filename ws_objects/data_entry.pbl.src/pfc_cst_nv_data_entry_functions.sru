...
FOR i = 1 TO upperbound(ls_sort_field)
	if lb_multi_tablE then			
		if left(ls_sort_field[i], 5) = 'sort4' then
			ls_sort_fields = ls_sort_field[i] + ls_sort_dir[i] 
		else
...
FOR i = 1 TO li_row_cnt
	IF ids_screen_fields.GetItemNumber( i, "sort_field" ) > 0 THEN
		...
		//---------Begin Commented by (Appeon)Stephen 06.20.2017 for Bug id 5703 - Getting sort error when trying to preview a page in Application Mapping. C/S Version 15.309 Build Date 5/11/2017--------
...
for i = 1 to ll_RowCount
	...
	*/
	...
	/*elseif 	ads_data.dataobject = "d_appl_tracking_for_batch_add" then 
		select application_name into :ls_app_name from app_hdr where app_id = :ll_app_id;*/ ////Commented by (Appeon)Stephen 07.21.2017 --add 'app_hdr_application_name' field
...
if li_add_row > 0 then
	...
	update ids set ids.sys_appaudit_id = :ll_audit_id_new + :li_add_row using sqlca;
	
	//---------Begin Added by (Appeon)Stephen 05.17.2017 for : Concentra - error when adding workflow--------
	if sqlca.sqlcode < 0 then
		ROLLBACK USING SQLCA;	
		MessageBox("of_appfield_audit", "Failed to update sys_audit table!")
		if IsValid(lds_sys_audit_appl) then destroy lds_sys_audit_appl
		Return -1
	end if
	//---------End Added ------------------------------------------------------
...
If of_get_app_setting("set_85","I") = 2 Then
	...
	*/
	...
	if ll_audit_id > ll_audit_id_new then ll_audit_id_new = ll_audit_id
	ll_audit_id_new +=1	
	update ids set ids.sys_appaudit_id = :ll_audit_id_new using sqlca;
	
	//---------Begin Added by (Appeon)Stephen 05.17.2017 for : Concentra - error when adding workflow--------
	if sqlca.sqlcode < 0 then
		ROLLBACK USING SQLCA;	
		MessageBox("of_appfield_audit_delete_last", "Failed to update sys_audit table!")
		if IsValid(lds_sys_audit_appl) then destroy lds_sys_audit_appl
		Return -1
	end if
	//---------End Added ------------------------------------------------------
...
ls_from_field = leftA(ls_from_field , len(ls_from_field ) - 1)  //Start Code Change ----08.17.2017 #V154 maha - remove last char(asc,desc)
